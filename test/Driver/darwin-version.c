...
// RUN: %clang -target armv7-apple-darwin -miphonsnax-version-min=11.0 -c -### %s 2> %t.err
// RUN: FileCheck --input-file=%t.err --check-prefix=CHECK-VERSION-IOS5 %s
// CHECK-VERSION-IOS5: invalid iOS deployment version '-miphonsnax-version-min=11.0'
...
// RUN: %clang -target arm64-apple-ios11.0 -miphonsnax-version-min=11.0 -c -### %s 2>&1 | \
...
// RUN: %clang -target armv7-apple-ios9.0 -miphonsnax-version-min=11.0 -c -Wno-invalid-ios-deployment-target -### %s 2>&1 | \
...
// RUN: env MACOSX_DEPLOYMENT_TARGET=10.5 IPHONSNAX_DEPLOYMENT_TARGET=2.0 \
...
// RUN: env IPHONSNAX_DEPLOYMENT_TARGET=2.3.1 \
// RUN:   %clang -target armv6-apple-darwin9 -c %s -### 2>&1 | \
...
// RUN: %clang -target x86_64-apple-macos -miphonsnax-version-min=9.1 -c %s -### 2>&1 | \
// RUN:   FileCheck --check-prefix=CHECK-VERSION-TNO-OSV2 %s
// CHECK-VERSION-TNO-OSV2: overriding '-miphonsnax-version-min=9.1' option with '--target=x86_64-apple-macos'
...
// RUN: %clang -target arm64-apple-ios10.1.0 -miphonsnax-version-min=10.1.0.1 -c %s -### 2>&1 | \
// RUN:   FileCheck --check-prefix=CHECK-VERSION-TNO-OSV4 %s
// CHECK-VERSION-TNO-OSV4: overriding '-miphonsnax-version-min=10.1.0.1' option with '--target=arm64-apple-ios10.1.0'
...
// RUN: env IPHONSNAX_DEPLOYMENT_TARGET=10.1 \
