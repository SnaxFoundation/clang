...
llvm::Constant *CodeGenModule::GetOrCreateLLVMFunction(
  ...
  if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
     if (FD->hasAttr<SnaxWasmImportAttr>())
        isWasmImport = true;
     if (FD->hasAttr<SnaxWasmEntryAttr>())
        isWasmEntry = true;
     if (FD->hasAttr<SnaxWasmABIAttr>())
        isWasmABI = true;
     if (FD->hasAttr<SnaxWasmActionAttr>())
        isWasmAction = true;
     if (FD->hasAttr<SnaxWasmNotifyAttr>())
        isWasmNotify = true;
  ...
  if (isWasmImport)
   F->addFnAttr("snax_wasm_import", "true");

  if (isWasmEntry)
   F->addFnAttr("snax_wasm_entry", "true");
     
  if (isWasmABI)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("snax_wasm_abi", FD->getWasmABI().c_str());
     }
  if (isWasmAction)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("snax_wasm_action", FD->getSnaxWasmAction().c_str());
     }
  if (isWasmNotify)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("snax_wasm_notify", FD->getSnaxWasmNotify().c_str());
     }
